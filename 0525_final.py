        ...
        # Task 2: 정지표지판, 느린표지판
            ...
            if not detected:
                for box in yolo_result_task3.boxes:
                    ...
                    # else:
                    #     turn_right = True    
                    if ((label in ['Car', 'Bus'] and bh > 0.38) or (label == 'Motorcycle' and bh > 0.24)):
                        print(bh)
                    # if label in ['Car', 'Bus', 'Motorcycle']: 
                        ...
                        if turn_left:
                            ...
                            run_motion(steer_right, speed, 0.85)       # 우회전
                            run_motion(steer_straight,speed_boost,0.32)
            ...
            if not detected:
                for box in yolo_result_task4.boxes:
                    ...
                    height = int(y2 - y1)
                    if label in ['Red','Green'] and not sig_task4_clear:
                        if height > 40:
                            if task_four_stop_and_go and label == "Green":
                                sig_task4_clear = True

                            elif sig_task4_clear and cur_sig == "Red":
                                speed = 0.0
                                steering *= 0.3
                                task_four_stop_and_go=True
                    ...
                    print(f" - {label}: (x1={int(x1)}, y1={int(y1)}, x2={int(x2)}, y2={int(y2)}), height={height}px")
                    # cls_id = int(box.cls[0])
                    # label = class_names[cls_id]
                    # cx, cy, bw, bh = box.xywhn[0].tolist()

                    if label in ['Left', 'Straight', 'Right'] and height > 45:
                        detected = True
                        if label == 'Left':
                            is_left = True
                        elif label == 'Straight':
                        ...
                        break
                if detected and is_left and sig_task4_clear:
                    speed = 0.18
                    steer_straight = 0.0
                    steer_left = -3.0
                ...
                elif detected and is_right and sig_task4_clear:
                    speed = 0.18
                    steer_straight = 0.0
                    steer_right = 3.0
                ...
                elif detected and is_straight and sig_task4_clear:
                    speed = 0.2
                    steer_straight = 0.0
